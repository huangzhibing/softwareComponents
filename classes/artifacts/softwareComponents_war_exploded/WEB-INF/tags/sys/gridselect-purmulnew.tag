...
$(document).ready(function(){
	$("#${id}Button, #${id}Names").click(function(){
		...
		top.layer.open({
		    ...
		    //获取name的值,得到gridselect标签隐藏input的对象的属性
		     var name='${name}'.split(".");
		     var nam=name[name.length-1];
		    //获取lableName的值，得到gridselect标签显示input的对象的属性
		    ...
		    // 设置gridselectr标签隐藏input的值
		       ...
		       for(var k=0;k<targetId.length;k++){		    	   
			    	  ...
			    	  if(tar=="costPrice"&&items[0][tar]=="null"){
							$(ht+"_"+targetId[k]).val("");
						}else{
			    	      var bean = tar.split(".");
			    	      if(bean.length == 1) {
                              $(ht + "_" + targetId[k]).val(items[0][bean]);
                          }else if(bean.length > 1 && bean[1] == "code"){
                              $("#${id}Names").val(items[0][bean[0]][bean[1]]);
                              $(ht + "_" + targetId[k]).val(items[0][bean[0]][bean[1]]);
						  }else if (bean.length == 2){
                              $(ht + "_" + targetId[k]).val(items[0][bean[0]][bean[1]]);
                          }else{
                              $(ht + "_" + targetId[k]).val(items[0][bean[0]][bean[1]][bean[2]]);
                          }
						}
