...
<%@ attribute name="param1" type="java.lang.String" required="true" description="url传递参数"%>
<%@ attribute name="param1value" type="java.lang.String" required="true" description="url传递参数值"%>
...
<%@ attribute name="disabled" type="java.lang.String" required="false" description="是否限制选择，如果限制，设置为disabled"%>
	...
	<div class="input-group" style="width: 100%">
		<input id="${id}Names"  name="${labelName}s" ${allowInput?'':'readonly="readonly"'}  onchange="setVal(this)" type="text" value="${labelValue}" data-msg-required="${dataMsgRequired}"
...
$(document).ready(function(){
	$("#${id}Button, #${id}Names").click(function(){
		...
		top.layer.open({
		    ...
		    content: "${ctx}/tag/gridselect?url="+encodeURIComponent("${url}?"+"${param1}="+param1value)+"&fieldLabels="+encodeURIComponent("${fieldLabels}")+"&fieldKeys="+encodeURIComponent("${fieldKeys}")+"&searchLabels="+encodeURIComponent("${searchLabels}")+"&searchKeys="+encodeURIComponent("${searchKeys}")+"&isMultiSelected=${isMultiSelected? true:false}",
		    btn: ['确定', '关闭'],
		    yes: function(index, layero){
			 ...
			 if(labelName.length==3){
               ...
               $("#supId").val(items[0].account.accountCode);
               $("#supNames").val(items[0].account.accountCode);
               console.log( $("#supId").val());
	...
	function addValue(items,targetField,targetId){				
			for(var i=0;i<targetField.length;i++){
				...
				if(ind=="taxRatio"){
					var tId=targetId[i];			
					$("#"+tId).val(((items[0][ind])*100).toFixed(0)+'%');
				}else{
					var tId=targetId[i];			
					$("#"+tId).val(items[0][ind]);
				}
