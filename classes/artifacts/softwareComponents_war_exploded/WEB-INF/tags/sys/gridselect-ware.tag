            ...
            top.layer.open({
                ...
                yes: function(index, layero){
                    ...
                    for(var i=0; i<items.length; i++){
                        var item = items[i];
                        console.log(item)
                        <%--console.log('item${fn:substring(labelName, fns:lastIndexOf(labelName, '.'), fn:length(labelName))}')--%>
                        ids.push(item.wareID);
...
//							 console.log(fieldArr[j])
                    ...
                    if('${labelField}'){
                        ...
                        if(document.getElementById('${id}d')!=null)
                            $("#${id}d").val(fieldValue.join(","));
                    }
