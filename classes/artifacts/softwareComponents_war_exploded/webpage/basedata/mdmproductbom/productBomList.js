...
$(document).ready(function() {
	$('#productBomTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/mdmproductbom/productBom/data",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/mdmproductbom/productBom/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该产品结构维护记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/mdmproductbom/productBom/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
					return "<a href='${ctx}/mdmproductbom/productBom/form?id="+row.id+"'>"+row.product.item.code+"</a>";
		        }
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/mdmproductbom/productBom/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该产品结构维护记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/mdmproductbom/productBom/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#productBomTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/mdmproductbom/productBom/form?id=" + getIdSelections();
  }
