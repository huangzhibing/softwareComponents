 ...
 */
 	function button_buyer(){
            ...
            top.layer.open({
                ...
                auto:true,
                name:'friend',
                content: "${ctx}/tag/gridselect?url="+encodeURIComponent("${ctx}/group/groupQuery/buyersdata")+"&fieldLabels="+encodeURIComponent("采购员编码|采购员名称")+"&fieldKeys="+encodeURIComponent("user.no|buyername")+"&searchLabels="+encodeURIComponent("采购员编码|采购员名称")+"&searchKeys="+encodeURIComponent("user.no|buyername")+"&isMultiSelected=false",
                btn: ['确定', '关闭'],
                yes: function(index, layero){
                    ...
                    for(var i=0;i<idsPre.length;i++){
                        $(idsPre[i]+"_buyerIdNames").val(items[0].user.no);
                        $(idsPre[i]+"_buyerIdId").val(items[0].id);
                        $(idsPre[i]+"_buyerName").val(items[0].user.name);
                    }
...
<div class="row">
	...
	<div class="panel panel-primary">
		...
		<form:form id="inputForm" modelAttribute="purPlanMain" action="${ctx}/purplan/purPlanMainOrder/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
...
<div class="table-responsive">
			<table class="table table-striped table-bordered table-condensed " style="min-width: 3000px"   >
				...
				<thead>
					<tr>
						...
						<th>移动平均价</th>
						<th>安全库存量</th>
						<th><font color="red">*</font>采购员编码</th>
						<th><font color="red">*</font>采购员名称</th>
						<th>供应商编号</th>
						<th>供应商名称</th>
						<th>备注</th>
			...
			<script type="text/template" id="purPlanDetailTpl">//<!--
				<tr id="purPlanDetailList{{idx}}">
					...
					<td>
						<input id="purPlanDetailList{{idx}}_planQty" name="purPlanDetailList[{{idx}}].planQty" type="text" value="{{row.planQty}}" readonly="true" onchange="qtySum(this)"   class="form-control required number"/>
					</td>
					...
					<td>
						<sys:gridselect-pursup-new url="${ctx}/group/groupQuery/buyersdata" id="purPlanDetailList{{idx}}_buyerId" name="purPlanDetailList[{{idx}}].buyerId.id" value="{{row.buyerId.id}}" labelName="purPlanDetailList{{idx}}.buyerId.user.no" labelValue="{{row.buyerId.user.no}}"
							 title="选择采购员编码" cssClass="form-control required " fieldLabels="采购员编码|采购员名称" fieldKeys="user.no|buyername" searchLabels="采购员编码|采购员名称" searchKeys="user.no|buyername" targetId="buyerName" targetField="buyername" ></sys:gridselect-pursup-new>
					</td>
					
					
					<td>
						<input id="purPlanDetailList{{idx}}_buyerName" name="purPlanDetailList[{{idx}}].buyerName" type="text" value="{{row.buyerName}}"    class="form-control required"/>
					</td>
		...
		<c:when test="${isEdit||isAdd}">
		        ...
		        <div class="col-lg-2">
		             <div class="form-group text-center">
		                 <div>
		                     <input type="button" id="save_btn" class="btn btn-primary btn-block btn-lg btn-parsley" onclick="button_buyer()"  value="采购员批量设置"></input>
		        ...
		        <div class="col-lg-2">
		             <div class="form-group text-center">
		                 <div>
		                     <input type="button" id="submit_btn" class="btn btn-primary btn-block btn-lg btn-parsley" onclick="button_submit()" data-loading-text="正在下达..." value="下 达"></input>
		                 </div>
		             </div>
		        </div>
		        <div class="col-lg-1"></div>
		...
		<c:otherwise>
				<div class="col-lg-4"></div>
		         ...
		         <div class="col-lg-4">
		             <div class="form-group text-center">
		                 <div>
		                     <a  class="btn btn-primary btn-block btn-lg btn-parsley"  onclick="histoty.go(-1)" data-loading-text="正在返回..." >返 回</a>
		                 </div>
		             </div>
		        </div>
		</c:otherwise>
