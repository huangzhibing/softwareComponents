...
<head>
	...
	<script type="text/javascript">
		...
		//	$("#inputForm").submit();
		$.post("${ctx}/contractmain/contractMainFreeze/save",$('#inputForm').serialize(),function(data){			
			jp.success(data.msg);
		});
		}
		...
		function closeContract(){
			$('#flag').val('no');
			$("#inputForm").attr('action','${ctx}/contractmain/contractMainClose/save');    
	    	$("#inputForm").submit();
		}
	</script>
</head>
...
<div class="row">
	...
	<div class="panel panel-primary">
		...
		<form:form id="inputForm" modelAttribute="contractMain" action="${ctx}/contractmain/contractMainFreeze/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<sys:message content="${message}"/>	
				...
				<div class="form-group">
					...
					<div class="col-sm-3">						
							<div class='input-group form_datetime' id='bdate'>
			                    <input type='text'  name="bdate" class="form-control required" readOnly="true"  value="<fmt:formatDate value="${contractMain.bdate}" pattern="yyyy-MM-dd"/>"/>
				...
				<div class="form-group">
					...
					<div class='input-group form_datetime' id='advanceDate'>
			                    <input type='text'  name="advanceDate" class="form-control"  readOnly="true" value="<fmt:formatDate value="${contractMain.advanceDate}" pattern="yyyy-MM-dd"/>"/>
			                    <span class="input-group-addon">
			                        <span class="glyphicon glyphicon-calendar"></span>
			                    </span>
			                </div>	
					...
					<div class='input-group form_datetime' id='endDate'>
			                    <input type='text'  name="endDate" class="form-control" readOnly="true"  value="<fmt:formatDate value="${contractMain.endDate}" pattern="yyyy-MM-dd"/>"/>
			                    <span class="input-group-addon">
			                        <span class="glyphicon glyphicon-calendar"></span>
			                    </span>
			                </div>	
		...
		<div class="table-responsive" style="max-height:500px">
			...
			<script type="text/template" id="contractDetailTpl">//<!--
				<tr id="contractDetailList{{idx}}">
					...
					<td>
						<input id="contractDetailList{{idx}}_itemTexture" name="contractDetailList[{{idx}}].itemTexture" type="text" value="{{row.itemTexture}}"  readOnly="true" readOnly="true"  class="form-control"/>
					</td>
		...
		<c:if test="${fns:hasPermission('contractmainfreeze:contractMain:edit') || isAdd}">
				<div class="row">
					<div class="col-lg-2"></div>
			        <div class="col-lg-3">
			             <div class="form-group text-center">
			                 <div>
			                 	 <button class="btn btn-danger btn-block btn-lg btn-parsley" data-loading-text="正在提交..." onclick="$('#flag').val('yes')">冻结</button>   		                 
			                 </div>
			             </div>
			        </div>
			        <div class="col-lg-2"></div>
			        ...
			        <div class="form-group text-center">
			                 <div>
	                             <a href= "#" class="btn btn-success btn-block btn-lg btn-parsley" data-loading-text="正在提交..." onclick="closeContract()">结案</a>		                
	                              <%-- 	  <a href= "${ctx}/contractmain/contractMainFreeze" class="btn btn-primary btn-block btn-lg btn-parsley" data-loading-text="正在保存...">返回</a>                  		                 
			                 --%>
			                 </div>
			             </div>
			        </div>
