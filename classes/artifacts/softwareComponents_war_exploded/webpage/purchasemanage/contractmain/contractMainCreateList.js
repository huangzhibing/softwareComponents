...
$(document).ready(function() {
	$('#contractMainTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/contractmain/contractMainCreate/data",
       ...
       //        contextMenu: '#context-menu',
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/contractmain/contractMainCreate/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该采购合同管理记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/contractmain/contractMainCreate/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/contractmain/contractMainCreate/form?id="+row.id+"'>"+value+"</a>";
		         }
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/contractmain/contractMainCreate/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该采购合同管理记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/contractmain/contractMainCreate/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#contractMainTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/contractmain/contractMainCreate/form?id=" + getIdSelections();
  }
...
//采购员标签内函数
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/contractmain/contractMainCreate/detail?id="+row.id, function(contractMain){
