...
$(document).ready(function() {
	$('#applyMainTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/applymain/applyMain/data",
                   	...
                   	window.location = "${ctx}/applymain/applyMain/form?id=" + row.id;
                        ...
                        jp.confirm('确认要删除该采购需求记录吗？', function(){
                       	...
                       	jp.get("${ctx}/applymain/applyMain/delete?id="+row.id, function(data){
                   	  		if(data.success){
                   	  			$('#applyMainTable').bootstrapTable('refresh');
                   	  			jp.success(data.msg);
                   	  		}else{
                   	  			jp.error(data.msg);
                   	  		}
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/applymain/applyMain/form?id="+row.id+"'>"+value+"</a>";
		         }
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#applyMainTable').bootstrapTable("toggleView");
		}
	  
	  $('#applyMainTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
                'check-all.bs.table uncheck-all.bs.table', function () {
            $('#remove').prop('disabled', ! $('#applyMainTable').bootstrapTable('getSelections').length);
            $('#edit').prop('disabled', $('#applyMainTable').bootstrapTable('getSelections').length!=1);
            $('#detail').prop('disabled', $('#applyMainTable').bootstrapTable('getSelections').length!=1);
        });
  ...
  function getIdSelections() {
        return $.map($("#applyMainTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
