...
<script>
 $(document).ready(function() {
    $('#mpsPlanTable').bootstrapTable({
        ...
        onContextMenuItem: function(row, $el){
            if($el.data("item") == "edit"){
                window.location = "${ctx}/ppc/mpsPlan/form?id=" + row.id;
            } else if($el.data("item") == "delete"){
                jp.confirm('确认要删除该MPSPlan记录吗？', function(){
                    jp.loading();
                    jp.get("${ctx}/ppc/mpsPlan/delete?id="+row.id, function(data){
                        if(data.success){
        ...
        ,{
            ...
            ,formatter:function(value, row , index){
                return "<a href='${ctx}/ppc/mpsPlan/form?id="+row.id+"'>"+value+"</a>";
            }
    ...
    window.location='${ctx}/ppc/mpsPlan/import/template';
},
    ...
    jp.confirm('确认要删除该主生产计划记录吗？', function(){
        jp.loading();
        jp.get("${ctx}/ppc/mpsPlan/deleteAll?ids=" + getIdSelections(), function(data){
            if(data.success){
                $('#mpsPlanTable').bootstrapTable('refresh');
                jp.success(data.msg);
            }else{
    ...
    function load(){
    jp.loading("读取中。。。");
    jp.get("${ctx}/ppc/mpsPlan/loadOrder",function(data){
        if (data.success) {
            jp.success(data.msg);
            $('#mpsPlanTable').bootstrapTable("refresh");//刷新bootstrapTable
        }
    })
}

    function edit(){
    window.location = "${ctx}/ppc/mpsPlan/form?id=" + getIdSelections();
}
