...
$(document).ready(function() {
	$('#realAccountCheckTable').bootstrapTable({
		  ...
		  //请求方法
    	       ...
    	       detailView: true,
    	       	//显示详细内容函数
	           detailFormatter: "detailFormatter4",
    	       //最低显示2行
                   	...
                   	window.location = "${ctx}/peopleRealaccount/realAccount/form?id=" + row.id;
                        ...
                        jp.confirm('确认要删除该人工凭证单据管理记录吗？', function(){
                       	...
                       	jp.get("${ctx}/realaccount/realAccount/delete?id="+row.id, function(data){
                   	  		if(data.success){
                   	  			$('#realAccountCheckTable').bootstrapTable('refresh');
                   	  			jp.success(data.msg);
                   	  		}else{
                   	  			jp.error(data.msg);
                   	  		}
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#realAccountCheckTable').bootstrapTable("toggleView");
		}
	  
	  $('#realAccountCheckTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
                'check-all.bs.table uncheck-all.bs.table', function () {
            $('#remove').prop('disabled', ! $('#realAccountCheckTable').bootstrapTable('getSelections').length);
            $('#jihe').prop('disabled', $('#realAccountCheckTable').bootstrapTable('getSelections').length!=1);
        });
  ...
  function getIdSelectionsbyjihe() {
        return $.map($("#realAccountCheckTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
  ...
  function jihe(flag){
	  window.location = "${ctx}/peopleRealaccount/realAccount/form?id=" + getIdSelectionsbyjihe()+"&flag="+flag;
  }
  ...
  function detailFormatter4(index, row) {
