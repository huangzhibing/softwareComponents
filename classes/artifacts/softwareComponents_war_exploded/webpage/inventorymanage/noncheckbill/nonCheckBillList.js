...
$(document).ready(function() {
	$('#billMainTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/noncheckbill/nonCheckBill/data",
	  ...
	  $('#billMainTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            // $('#add').prop('disabled', ! $('#billMainTable').bootstrapTable('getSelections').length);
            $('#add').prop('disabled', $('#billMainTable').bootstrapTable('getSelections').length==0);
        });
  ...
  function update(){
      if( $('#billMainTable').bootstrapTable('getSelections').length == 0){
          jp.alert("至少选中一条记录")
          return false
      }

		jp.confirm('确认要更新单据记录吗？', function(){
			jp.loading();
			jp.get("${ctx}/noncheckbill/nonCheckBill/update?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#billMainTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
