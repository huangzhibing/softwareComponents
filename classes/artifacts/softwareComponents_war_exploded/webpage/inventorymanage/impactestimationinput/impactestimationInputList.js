...
$(document).ready(function() {
        $('#billMainTable').bootstrapTable({
            ...
            url: "${ctx}/impactestimationinput/impactestimationInput/data?type=${type}",
            ...
            onContextMenuItem: function(row, $el){
                if($el.data("item") == "edit"){
                    ...
                    jp.confirm('确认要删除该单据记录吗？', function(){
                        jp.loading();
                        jp.get("${ctx}/impactestimationinput/impactestimationInput/delete?id="+row.id, function(data){
                            if(data.success){
            ...
            columns: [
                ...
                ,{
                    ...
                    ,formatter:function(value, row , index){
                        if('${type}' === 'Input') {
                            return "<a href='${ctx}/impactestimationinput/impactestimationInput/form?type=Input&id=" + row.id + "'>" + value + "</a>";
                        }
                        if('${type}' === 'Query') {
                            return "<a href='${ctx}/impactestimationinput/impactestimationInput/form?type=Query&id=" + row.id + "'>" + value + "</a>";
                        }
                        if('${type}' === 'Post') {
                            return "<a href='${ctx}/impactestimationinput/impactestimationInput/form?type=Post&id=" + row.id + "'>" + value + "</a>";
                        }
                    }
                }
                ,{
        ...
        $("#btnImport").click(function(){
            jp.open({
                ...
                btn1: function(index, layero){
                    window.location='${ctx}/impactestimationinput/impactestimationInput/import/template';
                },
...
function deleteAll(){
    ...
    jp.confirm('确认要删除该单据记录吗？', function(){
        jp.loading();
        jp.get("${ctx}/impactestimationinput/impactestimationInput/deleteAll?ids=" + getIdSelections(), function(data){
            if(data.success){
                $('#billMainTable').bootstrapTable('refresh');
                jp.success(data.msg);
            }else{
...
function edit(){
    window.location = "${ctx}/impactestimationinput/impactestimationInput/form?id=" + getIdSelections();
}
