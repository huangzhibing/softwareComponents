...
$(document).ready(function() {
	$('#billMainTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/inventoryprofit/inventoryProfit/data?type=${type}",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/inventoryprofit/inventoryProfit/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该单据记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/inventoryprofit/inventoryProfit/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	if('${type}' === 'input') {
                        return "<a href='${ctx}/inventoryprofit/inventoryProfit/form?type=input&id=" + row.id + "'>" + value + "</a>";
                    }
                    if('${type}' === 'query') {
                        return "<a href='${ctx}/inventoryprofit/inventoryProfit/form?type=query&id=" + row.id + "'>" + value + "</a>";
                    }
                    if('${type}' === 'post') {
                        return "<a href='${ctx}/inventoryprofit/inventoryProfit/form?type=post&id=" + row.id + "'>" + value + "</a>";
                    }
		         }
  ...
  function edit(){
	  window.location = "${ctx}/inventoryprofit/inventoryProfit/form?id=" + getIdSelections();
  }
