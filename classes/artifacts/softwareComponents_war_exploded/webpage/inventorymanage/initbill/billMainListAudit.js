...
$(document).ready(function() {
	$('#billMainTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/initbill/initBill/data",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/initbill/initBill/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该单据记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/initbill/initBill/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/initbill/initBill/formAudit?id="+row.id+"'>"+value+"</a>";
		         }
...
//		    }
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/initbill/initBill/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该单据记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/initbill/initBill/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#billMainTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/initbill/initBill/form?id=" + getIdSelections();
  }
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/initbill/initBill/detail?id="+row.id, function(billMain){
