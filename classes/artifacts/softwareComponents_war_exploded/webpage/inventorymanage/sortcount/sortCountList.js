...
$(document).ready(function() {
	$('#sortCountTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/sortcount/sortCount/data",
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#sortCountTable').bootstrapTable("toggleView");
		}
	  
	  $('#sortCountTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
                'check-all.bs.table uncheck-all.bs.table', function () {
            $('#remove').prop('disabled', ! $('#sortCountTable').bootstrapTable('getSelections').length);
            $('#edit').prop('disabled', $('#sortCountTable').bootstrapTable('getSelections').length!=1);
        });
	  ...
	  $("#search").click("click", function(row) {// 绑定查询按扭
  ...
  function getIdSelections() {
        return $.map($("#sortCountTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
