...
$(document).ready(function() {
	$('#processBatchTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/processroutine/processBatch/data",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/processroutine/processBatch/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该车间加工路线管理记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/processroutine/processBatch/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
			   ...
			   ,formatter:function(value, row , index){
				   return "<a href='${ctx}/processroutine/processBatch/form?id="+row.id+"'>"+value+"</a>";
			   }
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该车间加工路线管理记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/processroutine/processBatch/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#processBatchTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/processroutine/processBatch/form?id=" + getIdSelections();
  }
