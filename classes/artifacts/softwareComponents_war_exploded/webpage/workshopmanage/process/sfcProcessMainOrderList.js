...
$(document).ready(function() {
	$('#sfcProcessMainTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/process/sfcProcessMain/orderData",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/process/sfcProcessMain/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该车间作业计划记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/process/sfcProcessMain/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,
			{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/process/sfcProcessMain/form?id="+row.id+"'>"+value+"</a>";
		         }
		    }
			,{
		        field: 'prodCode',
