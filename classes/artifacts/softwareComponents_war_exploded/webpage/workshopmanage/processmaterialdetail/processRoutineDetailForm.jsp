...
<head>
	...
	<script type="text/javascript">
		...
		function button_submit(){
		    jp.confirm("是否已完成这道工艺？",function () {
                var nowTime=jp.dateFormat(new Date(),"YYYY-MM-dd hh:mm:ss");
                $("#billstate").val("O");
                $("input[name$='.assembleTime']").each(function () {//记录装配时间
					$(this).val(nowTime);
                });
                $("input[name$='.assemblePersonID']").each(function () {//记录装配人员id
                    $(this).val('${fns:getUser().id}');
                });
                $("input[name$='.assemblePersonName']").each(function () {//记录装配人员
                    $(this).val('${fns:getUser().name}');
                });
                $("#planedate").val(nowTime);//记录这道工艺实际操作时间
				$("#inputForm").submit();
            });
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/processmaterialdetail/processRoutineDetail"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-3">
					<div class='input-group form_datetime' id='actualBDate'>
						<input type='text'  name="actualBDate" class="form-control "  value="<fmt:formatDate value="${processRoutineDetail.actualBDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
						<span class="input-group-addon">
			                        <span class="glyphicon glyphicon-calendar"></span>
			                    </span>
					</div>
			...
			<script type="text/template" id="processMaterialDetailTpl">//<!--
				<tr id="processMaterialDetailList{{idx}}">
					<td class="hide">
						...
						<input id="processMaterialDetailList{{idx}}_assembleTime" name="processMaterialDetailList[{{idx}}].assembleTime" type="hidden" value="{{row.assembleTime}}"/>
						<input id="processMaterialDetailList{{idx}}_assemblePersonID" name="processMaterialDetailList[{{idx}}].assemblePersonID" type="hidden" value="{{row.assemblePersonID}}"/>
						<input id="processMaterialDetailList{{idx}}_assemblePersonName" name="processMaterialDetailList[{{idx}}].assemblePersonName" type="hidden" value="{{row.assemblePersonName}}"/>
					</td>
					...
					<td>
						<input id="processMaterialDetailList{{idx}}_itemSN" name="processMaterialDetailList[{{idx}}].itemSN" type="text" value="{{row.itemSN}}"    class="form-control required"/>
					</td>
					...
					<td style=" width: 80px; ">
						<select id="processMaterialDetailList{{idx}}_hasQualityPro" name="processMaterialDetailList[{{idx}}].hasQualityPro" data-value="{{row.hasQualityPro}}"   class="form-control  required ">
							<%--<option value=""></option>--%>
					...
					<td>
						<input id="processMaterialDetailList{{idx}}_qualityProblem" name="processMaterialDetailList[{{idx}}].qualityProblem" type="text" value="{{row.qualityProblem}}"    class="form-control"/>
					</td>
			...
			<script type="text/javascript">
				...
				$(document).ready(function() {
					...
					for (var i=0; i<data.length; i++){
					    if(data[i].hasQualityPro==undefined){
					        data[i].hasQualityPro="N";
						}
		...
		<c:if test="${fns:hasPermission('processmaterialdetail:processRoutineDetail:edit') || isAdd}">
				<div class="col-lg-4"></div>
		        <div class="col-lg-3">
		             <div class="form-group text-center">
		                 <div>
		                     <input type="button" class="btn btn-primary btn-block btn-lg btn-parsley" data-loading-text="正在提交..." value="提 交" onclick="button_submit()" />
		                 </div>
		             </div>
		        </div>
		</c:if>
