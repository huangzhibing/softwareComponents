...
$(document).ready(function() {
	$('#sfcMaterialOrderTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/materialorder/sfcMaterialOrder/queryData",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/materialorder/sfcMaterialOrder/queryForm?id=" + row.id;
               ...
               },
			{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/materialorder/sfcMaterialOrder/queryForm?id="+row.id+"'>"+value+"</a>";
		         }
		    }
		   ...
		   /*,{
			   ...
			   formatter:function(value, row , index) {
					if(value=='Y'){
						return "<span style='color: red;'>缺料</span>";
					}else if(value=='N'){
						return "不缺料";
					}else{
						return value;
...
function print(){
    ...
    var selects=getIdSelections();
    if(selects.length==0){
    	jp.warning("请选择要打印的领料单记录!");
    	return false;
	}
    $("#printIframe").attr("src","${ctx}/materialorder/sfcMaterialOrder/print?id=" + (selects[0]));
