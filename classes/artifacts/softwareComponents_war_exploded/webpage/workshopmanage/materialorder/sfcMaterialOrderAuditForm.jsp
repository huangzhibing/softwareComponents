...
<head>
	...
	<script type="text/javascript">
		...
		function button_pass(){
            jp.confirm("确认通过审核？",function () {
                $("#billStateFlag").val("S");
                $('#inputForm').submit();
            });
		}
		function button_nopass(){
            jp.confirm("确认不通过审核？",function () {
                $("#billStateFlag").val("P");
                $('#inputForm').submit();
            });
		}
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/materialorder/sfcMaterialOrder/auditList"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		<div class="panel-body">
		<form:form id="inputForm" modelAttribute="sfcMaterialOrder" action="${ctx}/materialorder/sfcMaterialOrder/auditSave" method="post" class="form-horizontal">
		<form:hidden path="id"/>
			...
			<div class="form-group">
				...
				<div class="col-sm-3">
					...
					<div class='input-group form_datetime' id='editDate'>
						<input type='text'  name="editDate" readonly="true" class="form-control required"  value="<fmt:formatDate value="${sfcMaterialOrder.editDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
			...
			<div class="form-group">
				...
				<div class="col-sm-3">
					...
					<div class='input-group form_datetime' id='operDate'>
						<input type='text'  name="operDate" readonly="true" class="form-control required"  value="<fmt:formatDate value="${sfcMaterialOrder.operDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
						<span class="input-group-addon">
			                        <span class="glyphicon glyphicon-calendar"></span>
			                    </span>
					</div>
		...
		<c:if test="${fns:hasPermission('materialorder:sfcMaterialOrder:edit') || isAdd}">
				<div class="col-lg-2"></div>
				 <div class="col-lg-3">
		             <div class="form-group text-center">
		                 <div>
							 <input type="button" class="btn btn-primary btn-block btn-lg btn-parsley" onclick="button_pass()" data-loading-text="审核通过" value="审核通过"></input>
		                 </div>
		             </div>
		        </div>
			<div class="col-lg-1"></div>
		        <div class="col-lg-3">
		             <div class="form-group text-center">
		                 <div>
							 <input type="button" class="btn btn-primary btn-block btn-lg btn-parsley" onclick="button_nopass()" data-loading-text="审核不通过" value="审核不通过"></input>
		                 </div>
		             </div>
		        </div>
		</c:if>
