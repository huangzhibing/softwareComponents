				...
				<tr id="invCheckDetailList{{idx}}">
					...
					<td>
						<sys:gridselect-purmulforqms url="${ctx}/purinvcheckmain/invCheckMain/dataRequest" id="invCheckDetailList{{idx}}_item" name="invCheckDetailList[{{idx}}].item.id" value="{{row.item.id}}" labelName="invCheckDetailList{{idx}}.item.code" labelValue="{{row.item.code}}"
							 title="选择物料编号" cssClass="form-control required " targetId="itemName|itemSpecmodel|unitCode|itemCode" targetField="name|specModel|unit|code"  fieldLabels="物料编码|物料名称" isMultiSelected="true" fieldKeys="code|name" searchLabels="物料编码|物料名称" searchKeys="code|name" ></sys:gridselect-purmulforqms>
					</td>
