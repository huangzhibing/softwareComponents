...
/* 			for(var i = 0; i <= invCheckDetailRowIdx; i++){
				var noillegalQtyallowed = $("#invCheckDetailList"+i+"_checkQty").val();
				if(noillegalQtyallowed == "" || noillegalQtyallowed == 0){
					alert("请输入有效到货量！");
					return false;
				}
			} */
		...
		function addRowModify(list, idx, tpl, row, obj,targetField,targetId,nam,labNam){
            if((list == '#invCheckDetailList') && (idx == 1 || idx > 1)){
                jp.alert('只能选择一个物料！');
                return;
            }
    ...
    function qtyCtrl(conId){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                console.log(1*wcheckQty - 1*data)
				if(1*wcheckQty - 1*data > 0){
		...
		function selectPlan(planId){
			...
			$.ajax({
				...
				success:function(data){
				    ...
				    //清除原有的计划子表
				    var namebox = $("input[name^='plnboxName']");  //获取name值为boxName的所有input
                    for(i = 0; i < namebox.length; i++) {                      
                        $(namebox[i]).parent().parent().remove(); 
                        purPlanDetailRowIdx--;
                    }
		...
		<div class="col-sm-3" style="display:none">
			...
			<form:input path="priceSum" htmlEscape="false"    class="form-control " readonly="true"/>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<sys:gridselect-pursupwithplaceholder url="${ctx}/account/account/data" id="sup" name="sup" value="${invCheckMain.supId}"  placeholder="由合同信息带入"
						labelName="account.accountCode" labelValue="${invCheckMain.supId}"
							 ...
							 searchKeys="accountCode|accountName|taxRatio" ></sys:gridselect-pursupwithplaceholder>
					</div>
					<label class="col-sm-2 control-label"><font color="red">*</font>供应商名称：</label>
					<div class="col-sm-3">
						<form:input path="supName" htmlEscape="false"   placeholder="由合同信息带入"  class="form-control " readonly="true"/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label"><font color="red">*</font>税率：</label>
					<div class="col-sm-3">
						<input id="taxRatio" name="taxRatioNew" htmlEscape="false"    class="form-control required" value="<fmt:formatNumber value="${invCheckMain.taxRatio}" maxIntegerDigits="4" type="percent"/>" placeholder="由合同信息带入"/>
					</div>
				</div>
				<div class="form-group">
					...
					<sys:gridselect-pursup url="${ctx}/purinvcheckmain/invCheckMain/waredata" id="ware" name="ware" value="${wareHouse.wareID}" 
					         labelName="wareID" labelValue="${wareHouse.wareID}"
				...
				<div class="form-group">
					...
					<sys:gridselect-pursupforInvCheck url="${ctx}/group/groupQuery/buyersdata" id="buyerId" name="buyerId.user.id" value="${invCheckMain.buyerId}" 
					   labelName=".user.no" labelValue="${invCheckMain.buyerId}"
						...
						<%-- <form:input path="buyerId" htmlEscape="false"    class="form-control "/> --%>
					</div>
				...
				<div id = "displayContract" class="form-group">
					...
					labelName="billNum" labelValue="${invCheckMain.contractId}"
							 title="选择合同" cssClass="form-control required" fieldLabels="合同号|合同类型名称|采购员|供应商编号|供应商名称|税率" fieldKeys="billNum|contypeName|buyerName|account.accountCode|supName|taxRatio" 
							 searchLabels="合同号|合同类型名称" searchKeys="billNum|contypeName"  
							 targetId="contractName|supName|taxRatio" targetField="contypeName|supName|taxRatio" ></sys:gridselect-pursupforitem>
			...
			<script type="text/javascript">
				...
				function checkQtyCount(idx){
					...
					if(currentQty > maxQty){
						...
						priceCalculate();
						//$("#invCheckDetailList"+idx+"_frontQty").val(currentQty);//每次修改到货量都在frontQty中更新
					}			
				}
...
/* 					  if(tagtaxRatio == null || tagtaxRatio == "" || tagtaxRatio == 0){
					  	  tagrealSumTaxed = tagrealSum;//不含税金额
					  }else{
