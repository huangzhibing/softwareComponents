	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/purinvcheckmain/invCheckMain/listAudit"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		<div class="panel-body">
		<form:form id="inputForm" modelAttribute="invCheckMain" action="${ctx}/purinvcheckmain/invCheckMain/AuditSubmit" method="post" onsubmit="return changeState1()" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="tabs-container">
			...
			<script type="text/template" id="invCheckDetailTpl">//<!--
				<tr id="invCheckDetailList{{idx}}">
					...
					<td>
						<input id="invCheckDetailList{{idx}}_checkQty" name="invCheckDetailList[{{idx}}].checkQty" type="text" value="{{row.checkQty}}"    class="form-control required isFloatGtZero"/>
					</td>
					
					<td>
						<input id="invCheckDetailList{{idx}}_qmsFlag" name="invCheckDetailList[{{idx}}].qmsFlag" type="text" value="{{row.qmsFlag}}"    class="form-control required" readonly="true"/>
					</td>
					<td>
		...
		<c:if test="${fns:hasPermission('purinvcheckmain:invCheckMain:edit') || isAdd}">
		        ...
		        <div class="col-lg-3">
		             <div class="form-group text-center">
		                 <div>
		                     <input type ="submit" name="Submit" class="btn btn-primary btn-block btn-lg btn-parsley" value = "审核通过" data-loading-text="提交审核中..."/>
		                 </div>
		             </div>
		        </div>
		         <div class="col-lg-2"></div>
		        ...
		        <div class="form-group text-center">
		                 <div>
		                       <input type ="submit" name="Submit" onclick="changeState2()" class="btn btn-primary btn-block btn-lg btn-parsley" value = "审核不通过" data-loading-text="提交审核中..."/>
		                 </div>
		             </div>
		        </div>
		</c:if>
