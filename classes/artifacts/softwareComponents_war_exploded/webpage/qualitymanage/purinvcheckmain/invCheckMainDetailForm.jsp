...
<head>
	...
	function getNowFormatDate() {
		...
		//id属性值先放入按钮输入框
		$("#supNames").val($("#supId").val());
		$("#wareNames").val($("#wareId").val());
		$("#ioTypeNames").val($("#ioType").val());
		//改变输入状态
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/purinvcheckmain/invCheckMain"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		...
		<div class="col-sm-3" style="display:none">
			...
			<form:input path="priceSum" htmlEscape="false"    class="form-control " readonly="true"/>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
					<sys:gridselect-pursup url="${ctx}/purinvcheckmain/invCheckMain/waredata" id="ware" name="ware" value="${wareHouse.wareID}" 
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
					<sys:gridselect-pursup url="${ctx}/group/groupQuery/buyersdata" id="buyerId" name="buyerId.user.id" value="${invCheckMain.buyerId}" 
				...
				<div class="form-group">
					...
					labelName="ioType" labelValue="${purInvCheckMain.billType.ioType}" param1 = "wareID" param1value = "wareNames"
			...
			<script type="text/template" id="invCheckDetailTpl">//<!--
				<tr id="invCheckDetailList{{idx}}">
					<td class="hide">
						...
						<input id="invCheckDetailList{{idx}}_itemCode" name="invCheckDetailList[{{idx}}].itemCode" type="hidden" value="{{row.itemCode}}"/>
					...
					<td>
						<sys:gridselect-purmulforqms url="${ctx}/purinvcheckmain/invCheckMain/dataRequest" id="invCheckDetailList{{idx}}_item" name="invCheckDetailList[{{idx}}].item.id" value="{{row.item.id}}" labelName="invCheckDetailList{{idx}}.item.code" labelValue="{{row.item.code}}"
							 title="选择物料编号" cssClass="form-control required " targetId="itemName|itemSpecmodel|unitCode" targetField="name|specModel|unit"  fieldLabels="物料编码|物料名称" isMultiSelected="true" fieldKeys="code|name" searchLabels="物料编码|物料名称" searchKeys="code|name" ></sys:gridselect-purmulforqms>
					</td>
					...
					<td class="text-center" width="10px">
						{{#delBtn}}<span class="close" onclick="delRow(this, '#invCheckDetailList{{idx}}');invCheckDetailRowIdx = 0;" title="删除">&times;</span>{{/delBtn}}
					</td>
				</tr>//-->
			</script>
			<script type="text/javascript">
		...
		<c:if test="${fns:hasPermission('purinvcheckmain:invCheckMain:edit') || isAdd}">
		        ...
		        <div class="col-lg-1">
		             <div class="form-group text-center">
		                 <div>
		                     <input type ="submit" name="Submit" class="btn btn-primary btn-block btn-lg btn-parsley" value = "保存" data-loading-text="保存到货录入单中..."/>
		                 </div>
		             </div>
		        </div>
		        <div class="col-lg-1"></div>
		        <div class="col-lg-1">
		             <div class="form-group text-center">
		                 <div>
		                     <input type ="submit" name="Submit" class="btn btn-primary btn-block btn-lg btn-parsley" value = "提交" onclick = "changeSubmit()" data-loading-text="提交到货录入单中..."/>
		                 </div>
		             </div>
		        </div>
		        <div class="col-lg-1"></div>
		        ...
		        <div class="form-group text-center">
		                 <div>
		                      <a href= "${ctx}/purinvcheckmain/invCheckMain/list" class="btn btn-primary btn-block btn-lg btn-parsley">打印物料二维码</a>
		                 </div>
		             </div>
		        </div>
		         <div class="col-lg-1"></div>
		        ...
		        <div class="form-group text-center">
		                 <div>
		                      <a href= "${ctx}/purinvcheckmain/invCheckMain/list" class="btn btn-primary btn-block btn-lg btn-parsley">返回</a>
		                 </div>
		             </div>
		        </div>
		</c:if>
