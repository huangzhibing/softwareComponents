	...
	<script type="text/javascript">
		...
		function setSFC(id){
			...
			$.ajax({
				type:'get',
				datatype:'json',
				url:"${ctx}/purreport/purReport/findReInvDate?str="+id,
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/purreport/purReport/reInvlist"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		...
		<form:hidden path="userDeptCode" value="${fns:getUserOfficeCode()}"/>
				...
				<div class="form-group">
					<label class="col-sm-2 control-label"><font color="red">*</font>超期复验单编号：</label>
					<div class="col-sm-10">
						<sys:gridselect-sfc url="${ctx}/newinvcheckmain/newinvCheckMain/checkdata" id="reinvCheckMain" name="reinvCheckMain.id" value="${purReport.reinvCheckMain.id}" labelName="reinvCheckMain.billNum" labelValue="${purReport.reinvCheckMain.billNum}"
							 title="选择超期复验单编号" cssClass="form-control required" fieldLabels="超期复验单据号|核算期|制单日期|制单人名称" fieldKeys="billNum|periodId|makeDate|makeEmpname" searchLabels="超期复验单据号|核算期|制单日期|制单人名称" searchKeys="billNum|periodId|makeDate|makeEmpname" ></sys:gridselect-sfc>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<form:input path="checktName" htmlEscape="false"  value="超期复验" readonly="true" class="form-control "/>
					</div>
				</div>
				<div class="form-group">
			...
			<script type="text/template" id="purReportRSnTpl">//<!--
				<tr id="purReportRSnList{{idx}}">
					...
					<td style="width: 120px;">
						<input id="purReportRSnList{{idx}}_objSn" name="purReportRSnList[{{idx}}].objSn" type="text" value="{{row.objSn}}"  readonly="true"  class="form-control required"/>
					</td>
		...
		<c:if test="${fns:hasPermission('purreport:purReport:edit') || isAdd}">
		        ...
		        <div class="col-lg-6">
		             <div class="form-group text-center">
		                ...
		                --%>
		                	 <input type="submit" class="btn btn-primary" value="保存" onclick="changeFlag1()"> &nbsp;
							 
							<input type="submit" class="btn btn-primary" value="完成并提交" onclick="changeFlag()"> &nbsp;
		...
		function myclose(){
			//alert("close");
			window.location ="${ctx}/purreport/purReport/reInvlist";
		...
		function save(){
			...
			$.ajax({
				type:'POST',
				datatype:'text',
				url:"${ctx}/purreport/purReport/aajax",
				data:$("form").serializeArray(),
				contentType:"application/x-www-form-urlencoded",
				success:function(data){
		...
		function sub(){
			...
			var newUrl = "${ctx}/purreport/purReport/subSaveReInv";    //设置新提交地址
