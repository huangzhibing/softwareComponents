	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/purreport/purReport"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		...
		<form:hidden path="userDeptCode" value="${fns:getUserOfficeCode()}"/>
				...
				<div class="form-group">
					<label class="col-sm-2 control-label"><font color="red">*</font>采购报检单编号：</label>
					<div class="col-sm-10">
						<form:input path="checktName" value="${purReport.invCheckMain.billnum}" htmlEscape="false"  readonly="true" class="form-control "/>
			...
			<script type="text/template" id="purReportRSnTpl">//<!--
				<tr id="purReportRSnList{{idx}}">
					...
					<td style="width: 120px;">
						<input id="purReportRSnList{{idx}}_objSn" name="purReportRSnList[{{idx}}].objSn" type="text" value="{{row.objSn}}" readonly="true" onchange="changeNum(this)"  class="form-control required"/>
					</td>
					
					
					<td style="width: 100px;">

						<input id="purReportRSnList{{idx}}_objectDef" name="purReportRSnList[{{idx}}].objCode"  type="text" value="{{row.objCode}}"  readonly="true"  class="form-control required"/>

					</td>
					
					
					<td style="width: 100px;">
						<input id="purReportRSnList{{idx}}_objName" name="purReportRSnList[{{idx}}].objName"  type="text" value="{{row.objName}}"  readonly="true"  class="form-control required"/>
					</td>
					
					
					<td style="width: 60px;">
						<select id="purReportRSnList{{idx}}_checkResult" name="purReportRSnList[{{idx}}].checkResult" data-value="{{row.checkResult}}" class="form-control required  " readonly="true">
							<option value=""></option>
							<c:forEach items="${fns:getDictList('isQualified')}" var="dict">
								<option value="${dict.value}">${dict.label}</option>
							</c:forEach>
					...
					<td style="width: 100px;">
						<input id="purReportRSnList{{idx}}_matterType" name="purReportRSnList{{idx}}.matterType.mattername"  value="{{row.matterType.mattername}}" readonly="true" class="form-control "/>
					</td>
					...
					<td style="width: 100px;">
						...
						<input id="purReportRSnList{{idx}}_qCNorm" name="purReportRSnList{{idx}}.qCNorm.qcnormName"  value="{{row.qCNorm.qcnormName}}"  readonly="true" class="form-control "/>
			...
			<script type="text/javascript">
				...
				function myDelete(){
					jp.confirm('确认要删除该检验单记录吗？', function(){
						var id = $("#id").val();
						window.location = "${ctx}/purreport/purReport/mydeleteEle?id=" + id;
