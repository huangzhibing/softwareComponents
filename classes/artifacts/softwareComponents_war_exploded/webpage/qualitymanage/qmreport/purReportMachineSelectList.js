...
function doSubmit(qmformFrame,index){//回调函数，在编辑和保存动作时，供openDialog调用提交表单。
  ...
  for(var i=0;i<selectItems.length;i++){
	  ...
	  rsnrow.mserialno=selectItems[i].mserialno;
	  ...
	  if(selectItems[i].checkDate!=null){
      ...
      if(dumplicate < '0'){
          ...
          qmformFrame.importCurrentRow(rsnrow);
          console.log(rsnrow);
          count++;
       }else{
           repeat++;
...
$(document).ready(function() {
	$('#purReportTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/purreport/purQmReport/dataMachine",
...
/*  function getIdSelections() {
  ...
  */
  ...
  function detailFormatter(index, row,$detail) {
	  $.get("${ctx}/purreport/purQmReport/detailForMachineQm?id="+row.id, function(purReport){
		  ...
		  if(typeof(data1)=="undefined"){ 
			...
			for(var i=0;i<data1.length;i++){
				...
				data1[i].checkDate=row.checkDate;
				data1[i].checkPerson=row.checkPerson;
				data1[i].checkTime=row.checkTime;
