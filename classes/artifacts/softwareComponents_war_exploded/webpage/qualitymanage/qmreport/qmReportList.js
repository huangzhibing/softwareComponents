...
$(document).ready(function() {
	$('#qmReportTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/qmreport/qmReport/data",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/qmreport/qmReport/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该质量问题记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/qmreport/qmReport/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/qmreport/qmReport/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该质量问题记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/qmreport/qmReport/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#qmReportTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  			updateNotProcDisplay();
  ...
  function edit(){
	  window.location = "${ctx}/qmreport/qmReport/form?id=" + getIdSelections();
  }
  
  function itemDetail(){
	  window.location = "${ctx}/qmreport/qmReport/form?id=" + getIdSelections()+"&justSee=true";
  }
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/qmreport/qmReport/detail?id="+row.id, function(qmReport){
