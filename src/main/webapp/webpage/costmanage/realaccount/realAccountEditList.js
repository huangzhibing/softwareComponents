...
$(document).ready(function() {
                   ...
                   if($el.data("item") == "xiugai"){
                   	window.location = "${ctx}/realaccount/realAccount/form?id=" + row.id;
                        ...
                        jp.confirm('确认要删除该材料凭证单据管理记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/realaccount/realAccount/deleteBack?id="+row.id, function(data){
                   	  		if(data.success){
  ...
  function deleteAllbyxiugai(){
		jp.confirm('确认要删除该材料凭证单据管理记录吗ll？', function(){
			jp.loading();
			jp.get("${ctx}/realaccount/realAccount/deleteAllBack?ids=" + getIdSelectionsbyxiugai(), function(data){
         	  		if(data.success){
         	  			$('#realAccountEditTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function xiugai(flag){
	  window.location = "${ctx}/realaccount/realAccount/form?id=" + getIdSelectionsbyxiugai()+"&flag="+flag;
  }
