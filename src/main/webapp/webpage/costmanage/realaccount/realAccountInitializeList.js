...
$(document).ready(function() {
	$('#realAccountTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/realaccount/realAccount/dataAccounting?flag=${flag}",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/realaccount/realAccount/formAccounting?id=" + row.id;
               ...
               columns: [{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/realaccount/realAccount/formAccounting?flag=${flag}&id="+row.id+"'>"+value+"</a>";
		         }
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#realAccountTable').bootstrapTable("toggleView");
		}
	  
	  $('#realAccountTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
                'check-all.bs.table uncheck-all.bs.table', function () {
            $('#removebyzhidan').prop('disabled', ! $('#realAccountTable').bootstrapTable('getSelections').length);
            $('#edit').prop('disabled', $('#realAccountTable').bootstrapTable('getSelections').length!=1);
        });
  ...
  function getIdSelectionsbyzhidan() {
        return $.map($("#realAccountTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该材料凭证单据管理记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/realaccount/realAccount/deleteAll?ids=" + getIdSelectionsbyzhidan(), function(data){
         	  		if(data.success){
         	  			$('#realAccountTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
