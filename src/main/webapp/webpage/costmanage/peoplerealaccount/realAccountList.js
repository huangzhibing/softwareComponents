...
$(document).ready(function() {
	$('#realAccountTable').bootstrapTable({
		  ...
		  //请求方法
    	       ...
    	       detailView: true,
    	       	//显示详细内容函数
	           detailFormatter: "detailFormatter3",
    	       //最低显示2行
  ...
  function getIdSelectionsbyzhidan() {
        var selected = $("#realAccountTable").bootstrapTable('getSelections');
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该人工凭证单据管理记录吗？', function(){
			jp.loading();
			var ids = getIdSelectionsbyzhidan();
			console.log(ids);
			jp.get("${ctx}/peopleRealaccount/realAccount/deleteAll?ids=" +ids , function(data){
         	  		if(data.success){
         	  			$('#realAccountTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(flag){
	  window.location = "${ctx}/peopleRealaccount/realAccount/form?id=" + getIdSelections()+"&flag="+flag;
  }
