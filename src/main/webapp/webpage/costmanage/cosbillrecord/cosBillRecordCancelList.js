...
$(document).ready(function() {
	$('#billMainTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/cosbillrecord/cosBillRecord/dataforCancelCheck",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/billmain/billMain/Cancelform?id=" + row.id;
   			...
   			,{
   		        ...
   		        ,formatter:function(value, row , index){
   		        	return "<a href='${ctx}/cosbillrecord/cosBillRecord/Cancelform?id="+row.id+"'>"+value+"</a>";
   		         }
   			...
   			,{
   		        ...
   		        sortable: true,
   		        formatter:function(value, row , index){
   		        	console.log(${fns:toJson(fns:getDictList('cosBillStatus'))});
   		        	return jp.getDictLabel(${fns:toJson(fns:getDictList('cosBillStatus'))}, value, "-");
   		        }
   		    }
   			,{
   		        field: 'mpsPlanId',
  ...
  function delCosBillRecordAll(){
		jp.confirm('确认要撤销该单据记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/cosbillrecord/cosBillRecord/delCosBillRecordAll?ids=" + getIdSelections(), function(data){
       	  		if(data.success){
       	  			$('#billMainTable').bootstrapTable('refresh');
       	  			jp.success(data.msg);
       	  		}else{
