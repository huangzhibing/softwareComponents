...
$(document).ready(function() {
	$('#contractMainTable').bootstrapTable({
		  ...
		  //请求方法
	       ...
	       //    detailFormatter: "detailFormatter",
               ...
               url: "${ctx}/contractmain/contractMainWarning/data",
    ...
    function getTdValue(data){
        var tableId = document.getElementById("contractMainTable");
      //  console.log(data);
        var obj=data.rows;
        //转换成json格式
        var jsonObj = JSON.stringify(obj); 
     //   console.log(jsonObj);
        var josn =JSON.parse(jsonObj);
        for(var i = 1;i < tableId.rows.length;i++) {

            if((josn[i-1].pastTD)==true){
              tableId.rows[i].setAttribute("style","background: #FFFACD;");//过去三天内的标注颜色
            }
            if((josn[i-1].futureSD)==true){
                 tableId.rows[i].setAttribute("style","background: #D1EEEE;");//将来七天内的标注颜色
            }
        }
    }
	 ...
	 $('#arriveBeginDate').datetimepicker({
		 format: "YYYY-MM-DD"
     });

	$('#arriveEndDate').datetimepicker({
			 format: "YYYY-MM-DD"
	});
  ...
  function detailFormatter(index, row) {
	...
	function print(){		  
		var data = document.getElementById("contractMainTable").innerHTML;
	  	$("#printContent").html("");
	  	var thead = "<p align= 'center'>采购订单交期预警表</p>";
