...
<head>
	...
	<script type="text/javascript">
		...
		function qtyNull(id){
			...
			var preId = id.substring(0,idIndex+1);
			var qtyId = preId+"prodQty"; 
			var val = document.getElementById(qtyId).value;
			if(val==null||val==''){
				layer.alert("请先填写签订量");
				document.getElementById(preId+"prodPrice").value="";
				document.getElementById(preId+"prodPriceTaxed").value="";
			}
		}
		function calNoTax(id){
			var idIndex = id.indexOf("_");
			var preId = id.substring(0,idIndex+1);
			var qtyId = preId+"prodQty"; 
			var qty = document.getElementById(qtyId).value;
			var price = document.getElementById(preId+"prodPrice").value;
			var tax = document.getElementById("taxRatiod").value;
			if(tax==''||tax==null){
				layer.alert("请先填写税率");
				document.getElementById(preId+"prodPrice").value="";
				return;
			}
			document.getElementById(preId+"prodSum").value=(parseFloat(qty)*parseFloat(price)).toFixed(2);
			document.getElementById(preId+"prodPriceTaxed").value=((1+parseFloat(tax))*parseFloat(price)).toFixed(2);
			document.getElementById(preId+"prodSumTaxed").value=(parseFloat(qty)*(1+parseFloat(tax))*parseFloat(price)).toFixed(2);
		}
		function calTransSum(id,value){
			var idIndex = id.indexOf("_");
			var preId = id.substring(0,idIndex+1);
			var qtyId = preId+"prodQty";
			var qty = document.getElementById(qtyId).value;
			document.getElementById(preId+"transSum").value=parseFloat(qty)*parseFloat(value);
		}
		function calTax(id){
			/* var idIndex = id.indexOf("_");
			var preId = id.substring(0,idIndex+1);
			var qtyId = preId+"prodQty"; 
			var qty = document.getElementById(qtyId).value;
			var price = document.getElementById(preId+"prodPriceTaxed").value; */
			/* var tax = document.getElementById("taxRatiod").value;
			if(tax==''||tax==null){
				layer.alert("请先填写税率");
				return;
			} 
			document.getElementById(preId+"prodSumTaxed").value=parseFloat(qty)*parseFloat(price)*/;
		}
	</script>
</head>
<body>
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/salcontract/contract"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		<div class="panel-body">
		<form:form id="inputForm" modelAttribute="contract" action="${ctx}/salcontract/contract/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>	
					...
					<div class="col-sm-10">
						<p class="input-group">
							<div class='input-group form_datetime' id='signDate'>
			                    <input type='text'  name="signDate" class="form-control required"  value="<fmt:formatDate value="${contract.signDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
					...
					<div class="col-sm-10">
						<p class="input-group">
							<div class='input-group form_datetime' id='deliveryDate' >
			                    <input  type='text'  name="deliveryDate" class="form-control required"  value="<fmt:formatDate value="${contract.deliveryDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
			                    <span class="input-group-addon">
			                        <span class="glyphicon glyphicon-calendar"></span>
			                    </span>
			                </div>						            
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<sys:gridselect-modify extraField="ctrTypeCoded:ctrTypeCode" targetId="contractTypeNamed" targetField="ctrTypeName" url="${ctx}/contract/ctrTypeDef/data" id="contractType" name="contractType.id" value="${contract.contractType.id}" labelName="contractType.ctrTypeCode" labelValue="${contract.contractType.ctrTypeCode}"
				...
				<input style="display:none;" id="ctrTypeCoded" name="contractType.ctrTypeCode" value="${contract.contractType.ctrTypeCode}"/>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<sys:gridselect-modify extraField="accountCoded:accountCode" targetId="accountNamed" targetField="accountName" url="${ctx}/account/account/data?accTypeNameRu=客户" id="account" name="account.id" value="${contract.account.id}" labelName="account.accountCode" labelValue="${contract.account.accountCode}"
				...
				<input style="display:none;" id="accountCoded" name="account.accountCode" value="${contract.account.accountCode }"/>
				<div class="form-group">
				...
				<div class="form-group">
					<label class="col-sm-2 control-label"><font color="red">*</font>结算方式编码：</label>
					<div class="col-sm-10">
						<sys:gridselect-modify extraField="blncTypeCoded:blncTypeCode" targetId="blncTypeNamed" targetField="blncTypeName" url="${ctx}/blnctypedef/blncTypeDef/data" id="blncType" name="blncType.id" value="${contract.blncType.id}" labelName="blncType.blncTypeCode" labelValue="${contract.blncType.blncTypeCode}"
				...
				<input id="blncTypeCoded" style="display:none;" name="blncType.blncTypeCode" value="${contract.blncType.blncTypeCode}"/>
				<div class="form-group">
					<label class="col-sm-2 control-label"><font color="red">*</font>结算方式名称：</label>
					<div class="col-sm-10">
						<form:input id="blncTypeNamed" readOnly="true" path="blncTypeName" htmlEscape="false"    class="form-control "/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label"><font color="red">*</font>销售人员编码：</label>
					<div class="col-sm-10">
						<sys:userselect-modify extraId="personNo" targetId="personNamed" id="person" name="person.id" value="${contract.person.id}" labelName="person.no" labelValue="${contract.person.no}"
							    cssClass="form-control required"/>
					</div>
				</div>
				<form:input style="display:none;" id="personNo" readOnly="true" path="person.no" htmlEscape="false"    class="form-control "/>
				
				<div class="form-group">
					<label class="col-sm-2 control-label"><font color="red">*</font>销售人员姓名：</label>
					<div class="col-sm-10">
						<form:input id="personNamed" readOnly="true" path="person.name" htmlEscape="false"    class="form-control "/>
					</div>
				</div>
				<div class="form-group">
				...
				<div  style="display:none;">
					<form:input path="inputPerson.id" htmlEscape="false"    class="form-control "/>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<sys:userselect-modify targetId="endPsnNamed" id="endPerson" name="endPerson.id" value="${contract.endPerson.id}" labelName="endPerson.name" labelValue="${contract.endPerson.name}"
							    cssClass="form-control "/>
					</div>
				</div> --%>
				<%-- <div class="form-group">
					...
					<div class="col-sm-10">
						<p class="input-group">
							<div class='input-group form_datetime' id='endDate'>
			                    <input type='text'  name="endDate" class="form-control "  value="<fmt:formatDate value="${contract.endDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
			                    <span class="input-group-addon">
			                        <span class="glyphicon glyphicon-calendar"></span>
			                    </span>
			                </div>						            
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<sys:gridselect-modify extraField="payTypeCode:typeCode" targetId="payTypeNamed" targetField="typeName" url="${ctx}/paytypedef/payTypeDef/data" id="payType" name="payType.id" value="${contract.payType.id}" labelName="payType.typeCode" labelValue="${contract.payType.typeCode}"
				...
				<form:input style="display:none;" id="payTypeCode" readOnly="true" path="payType.typeCode" htmlEscape="false"    class="form-control "/>
				<div class="form-group">
				...
				<div class="form-group">
					<label class="col-sm-2 control-label"><font color="red">*</font>税率编码：</label>
					<div class="col-sm-10">
						<sys:gridselect-modify extraField="salTaxCode:salTaxCode" targetId="taxRatiod" targetField="salTaxRatio" url="${ctx}/saltaxratio/saleTaxRatio/data" id="saleTaxRatio" name="saleTaxRatio.id" value="${contract.saleTaxRatio.id}" labelName="saleTaxRatio.salTaxCode" labelValue="${contract.saleTaxRatio.salTaxCode}"
				...
				<form:input style="display:none;" id="salTaxCode" readOnly="true" path="saleTaxRatio.salTaxCode" htmlEscape="false"    class="form-control "/>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<form:input path="transfeeSum" htmlEscape="false"    class="form-control  isFloatGteZero"/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label">不含税总额：</label>
					<div class="col-sm-10">
						<form:input path="goodsSum" htmlEscape="false"    class="form-control  isFloatGteZero"/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label">含税总额：</label>
					<div class="col-sm-10">
						<form:input path="goodsSumTaxed" htmlEscape="false"    class="form-control  isFloatGteZero"/>
					</div>
				</div>
				<%-- <div class="form-group">
					<label class="col-sm-2 control-label">合同文本：</label>
					<div class="col-sm-10">
						<form:input path="itemContents" htmlEscape="false"    class="form-control "/>
					</div>
				</div> --%>
				<div class="form-group">
					<label class="col-sm-2 control-label">纸制合同号：</label>
					<div class="col-sm-10">
						<form:input path="paperCtrCode" htmlEscape="false"    class="form-control "/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label">备注：</label>
					<div class="col-sm-10">
						<form:input path="praRemark" htmlEscape="false"    class="form-control "/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label">合同模板编码：</label>
					<div class="col-sm-10">
						<sys:gridselect-modify targetId="ctrItemsModeld" targetField="itemName" extraField="itemOrderd:itemOrder;modelContent:itemContents" url="${ctx}/contractmodel/ctrItemsModel/data" id="ctrItemsModel" name="ctrItemsModel.id" value="${contract.ctrItemsModel.id}" labelName="ctrItemsModel.itemOrder" labelValue="${contract.ctrItemsModel.itemOrder}"
				...
				<input id="itemOrderd" name="ctrItemsModel.itemOrder" value="${contract.ctrItemsModel.itemOrder}" hidden/>
				<div class="form-group">
		...
		<div class="tabs-container">
            <ul class="nav nav-tabs">
                <li class="active"><a data-toggle="tab" href="#tab-2" aria-expanded="true">销售合同子表：</a>
                </li>
			...
			<script type="text/template" id="ctrFIleTpl">//<!--
				<tr id="ctrFIleList{{idx}}">
					...
					<td style="display:none;">
						<input id="ctrFIleList{{idx}}_fileCode" name="ctrFIleList[{{idx}}].fileCode" type="text" value="{{row.fileCode}}"    class="form-control "/>
					</td>
					
					
					<td style="display:none;">
						<input id="ctrFIleList{{idx}}_fileName" name="ctrFIleList[{{idx}}].fileName" type="text" value="{{row.fileName}}"    class="form-control "/>
					</td>
					
					
					<td>
						<input id="ctrFIleList{{idx}}_filePath" name="ctrFIleList[{{idx}}].filePath" type="hidden" value="{{row.filePath}}" maxlength="200" class="form-control"/>
						<sys:ckfinder input="ctrFIleList{{idx}}_filePath" type="files" uploadPath="/salcontract/contract" selectMultiple="true"/>
					</td>
				...
				<tr id="ctrItemList{{idx}}">
					...
					<td>
						<sys:gridselect-contract
							...
							extraField="ctrItemList{{idx}}_productCode:item.code;ctrItemList{{idx}}_prodCode:id;ctrItemList{{idx}}_prodName:itemNameRu;ctrItemList{{idx}}_prodSpec:itemSpec;ctrItemList{{idx}}_unitName:itemMeasure" 
							url="${ctx}/product/product/data" id="ctrItemList{{idx}}_prodCode" name="ctrItemList{{idx}}.prodCode" value="{{row.prodCode.id}}" labelName="ctrItemList{{idx}}.prodCode.item.code" labelValue="{{row.productCode}}"
					...
					<td>
						<input onkeyup="calNoTax(this.id)"  id="ctrItemList{{idx}}_prodQty" name="ctrItemList[{{idx}}].prodQty" type="text" value="{{row.prodQty}}"    class="form-control  isFloatGteZero"/>
					</td>
					
					
					<td>
						<input onkeyup="calNoTax(this.id);qtyNull(this.id)" onkeyup="qtyNull(this.id)" id="ctrItemList{{idx}}_prodPrice" name="ctrItemList[{{idx}}].prodPrice" type="text" value="{{row.prodPrice}}"    class="form-control  isFloatGteZero"/>
					</td>
					...
					<td>
						<input readOnly="true" onkeyup="calTax(this.id)" onkeyup="qtyNull(this.id)" id="ctrItemList{{idx}}_prodPriceTaxed" name="ctrItemList[{{idx}}].prodPriceTaxed" type="text" value="{{row.prodPriceTaxed}}"    class="form-control  "/>
					</td>
					<td>
					...
					<td>
						<input id="ctrItemList{{idx}}_pickQty" name="ctrItemList[{{idx}}].pickQty" type="text" readonly="true" value="0"     class="form-control  isFloatGteZero"/>
					</td>
					
					
					<td>
						<input onkeyup="calTransSum(this.id,this.value)" id="ctrItemList{{idx}}_transPrice" name="ctrItemList[{{idx}}].transPrice" type="text" value="{{row.transPrice}}"    class="form-control  isFloatGteZero"/>
					</td>
