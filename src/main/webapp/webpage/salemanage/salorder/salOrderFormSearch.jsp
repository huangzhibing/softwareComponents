	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/salorder/salOrder/salOrderSearch"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		...
		<sys:message content="${message}"/>	
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<sys:gridselect-salorder targetId="" targetField="" extraField="accountCoded:account.accountCode;accountNamed:account.accountName;deliveryDated:deliveryDate" 
						url="${ctx}/salcontract/contract/data" id="contract" name="contract.id" value="${salOrder.contract.id}" labelName="contract.contractCode" labelValue="${salOrder.contract.contractCode}"
					...
					<div class="col-sm-10">
						<p class="input-group">
							<div class='input-group form_datetime' id='signDate'>
			                    <input readOnly="readOnly" type='text'  name="signDate" class="form-control required"  value="<fmt:formatDate value="${salOrder.signDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
					...
					<div class="col-sm-10">
						<p class="input-group">
							<div class='input-group form_datetime' id='sendDate'>
			                    <input readOnly="true" type='text' id="deliveryDated"  name="sendDate" class="form-control required"  value="<fmt:formatDate value="${salOrder.sendDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
			                    <span class="input-group-addon">
			                        <span class="glyphicon glyphicon-calendar"></span>
			                    </span>
			                </div>						            
		...
		<div class="tabs-container">
			...
			<script type="text/template" id="salOrderAuditTpl">//<!--
				<tr id="salOrderAuditList{{idx}}">
					...
					<td>
						<input readOnly="true"   id="salOrderAuditList{{idx}}_auditCode" name="salOrderAuditList[{{idx}}].auditCode" type="text" value="{{row.auditCode}}"    class="form-control "/>
					</td>
					
					
					<td>
						<input readOnly="true"  id="salOrderAuditList{{idx}}_auditPerson" name="salOrderAuditList[{{idx}}].auditPerson" type="text" value="{{row.auditPerson}}"    class="form-control "/>
					</td>
					
					
					<td>
						<div class='input-group form_datetime' id="salOrderAuditList{{idx}}_auditDate">
		                    <input readOnly="true"  type='text'  name="salOrderAuditList[{{idx}}].auditDate" class="form-control "  value="{{row.auditDate}}"/>
		                    <span class="input-group-addon">
		                        <span class="glyphicon glyphicon-calendar"></span>
		                    </span>
		                </div>						            
					...
					<td>
						<input readOnly="true"  id="salOrderAuditList{{idx}}_auditNote" name="salOrderAuditList[{{idx}}].auditNote" type="text" value="{{row.auditNote}}"    class="form-control "/>
					</td>
					
					
					<td>
						<select disabled="disabled" id="salOrderAuditList{{idx}}_auditStat" name="salOrderAuditList[{{idx}}].auditStat" data-value="{{row.auditStat}}" class="form-control m-b  ">
							<option value=""></option>
			...
			<script type="text/template" id="salOrderItemTpl">//<!--
				<tr id="salOrderItemList{{idx}}">
					...
					<td>
						<input readOnly="true"   id="salOrderItemList{{idx}}_prodQty" name="salOrderItemList[{{idx}}].prodQty" type="text" value="{{row.prodQty}}"    class="form-control "/>
					</td>
					
					
					<td>
						<input readOnly="true"   id="salOrderItemList{{idx}}_prodPrice" name="salOrderItemList[{{idx}}].prodPrice" type="text" value="{{row.prodPrice}}"    class="form-control "/>
					</td>
					
					
					<td>
						<input readOnly="true"   id="salOrderItemList{{idx}}_prodSum" name="salOrderItemList[{{idx}}].prodSum" type="text" value="{{row.prodSum}}"    class="form-control "/>
					</td>
					
					
					<td>
						<input readOnly="true"   id="salOrderItemList{{idx}}_prodPriceTaxed" name="salOrderItemList[{{idx}}].prodPriceTaxed" type="text" value="{{row.prodPriceTaxed}}"    class="form-control "/>
					</td>
					
					
					<td>
						<input readOnly="true"   id="salOrderItemList{{idx}}_prodSumTaxed" name="salOrderItemList[{{idx}}].prodSumTaxed" type="text" value="{{row.prodSumTaxed}}"    class="form-control "/>
					</td>
		...
		<c:if test="${fns:hasPermission('salorder:salOrder:edit') || isAdd}">
		        ...
		        <div class="col-lg-6">
		             <div class="form-group text-center">
		                 <div>
		                     <a href="${ctx}/salorder/salOrder/salOrderSearch" class="btn btn-primary btn-block btn-lg btn-parsley" data-loading-text="正在提交...">返回</a>
		                 </div>
		             </div>
		        </div>
		</c:if>
