...
$(document).ready(function() {
	$('#mpsPlanQueryTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/mpsplanquery/mpsPlanQuery/data",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/mpsplanquery/mpsPlanQuery/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该主生产计划记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/mpsplanquery/mpsPlanQuery/delete?id="+row.id, function(data){
                   	  		if(data.success){
                   	  			$('#mpsPlanQueryTable').bootstrapTable('refresh');
                   	  			jp.success(data.msg);
                   	  		}else{
                   	  			jp.error(data.msg);
                   	  		}
			...
			,{
		        ...
		        /*,formatter:function(value, row , index){
		        	return "<a href='${ctx}/mpsplanquery/mpsPlanQuery/form?id="+row.id+"'>"+value+"</a>";
		         }*/
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#mpsPlanQueryTable').bootstrapTable("toggleView");
		}
	  
	  $('#mpsPlanQueryTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
                'check-all.bs.table uncheck-all.bs.table', function () {
            $('#remove').prop('disabled', ! $('#mpsPlanQueryTable').bootstrapTable('getSelections').length);
            $('#edit').prop('disabled', $('#mpsPlanQueryTable').bootstrapTable('getSelections').length!=1);
        });
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/mpsplanquery/mpsPlanQuery/import/template';
				  },
  ...
  function getIdSelections() {
        return $.map($("#mpsPlanQueryTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该主生产计划记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/mpsplanquery/mpsPlanQuery/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#mpsPlanQueryTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/mpsplanquery/mpsPlanQuery/form?id=" + getIdSelections();
  }
