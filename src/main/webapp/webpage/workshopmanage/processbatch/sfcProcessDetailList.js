...
$(document).ready(function() {
	$('#sfcProcessDetailTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/processbatch/sfcProcessDetail/data",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/processbatch/sfcProcessDetail/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该车间作业计划分批记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/processbatch/sfcProcessDetail/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
			   ...
			   ,formatter:function(value, row , index){
				   return "<a href='${ctx}/processbatch/sfcProcessDetail/form?id="+row.id+"'>"+value+"</a>";
			   }
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该车间作业计划分批记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/processbatch/sfcProcessDetail/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#sfcProcessDetailTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/processbatch/sfcProcessDetail/form?id=" + getIdSelections();
  }
