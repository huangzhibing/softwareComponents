...
$(document).ready(function() {
	$('#processRoutineDetailTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/processmaterial/processRoutineDetail/data",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/processmaterial/processRoutineDetail/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该车间物料安装单记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/processmaterial/processRoutineDetail/delete?id="+row.id, function(data){
                   	  		if(data.success){
		   ...
		   ,{
			   ...
			   ,formatter:function(value, row , index){
				   return "<a href='${ctx}/processmaterial/processRoutineDetail/form?id="+row.id+"'>"+value+"</a>";
			   }
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/processmaterial/processRoutineDetail/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该车间物料安装单记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/processmaterial/processRoutineDetail/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#processRoutineDetailTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/processmaterial/processRoutineDetail/form?id=" + getIdSelections();
  }
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/processmaterial/processRoutineDetail/detail?id="+row.id, function(processRoutineDetail){
    	var processRoutineDetailChild1RowIdx = 0, processRoutineDetailChild1Tpl = $("#processRoutineDetailChild1Tpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g,"");
		var data1 =  processRoutineDetail.processMaterialList;
...
<script type="text/template" id="processRoutineDetailChildrenTpl">//<!--
	<div class="tabs-container">
		<ul class="nav nav-tabs">
				<li class="active"><a data-toggle="tab" href="#tab-{{idx}}-1" aria-expanded="true">车间物料安装单</a></li>
		</ul>
