...
<head>
	...
	<script type="text/javascript">
        ...
        function changeDeal(obj){
            var pre=obj.id.split("_")[0];
            jp.loading("处理中。。。");
            jp.get("${ctx}/processmaterialdetail/processRoutineDetail/changeDeal?changeId=" + $("#"+pre+"_id").val(), function(data){
                if(data.success){
                    //$('#processRoutineDetailTable').bootstrapTable('refresh');
                    jp.warning("请扫入换件后的物料二维码！");
                    $(obj).attr("disabled",true);
                    $("#"+pre+"_itemSN").removeAttr("readonly");
                    //$("#"+pre+"_qualityProblem").removeAttr("readonly");
                    $("#"+pre+"_itemSN").val("");
                }else{
                    jp.error("数据出错，请刷新当前页面后重试！");
                }
            });
			...
			<div class="panel panel-primary">
				<div class="panel-heading">
					<h3 class="panel-title">
						<a class="panelButton" href="${ctx}/processmaterialdetail/processRoutineDetail/changeList"><i class="ti-angle-left"></i> 返回</a>
					</h3>
				</div>
				...
				<tr id="processMaterialDetailList{{idx}}">
					...
					<td>
						<input id="processMaterialDetailList{{idx}}_itemSN" name="processMaterialDetailList[{{idx}}].itemSN" type="text" value="{{row.itemSN}}"  readonly="true"   class="form-control required"/>
					</td>
				...
				<%--	<td style=" width: 80px; ">
					...
					<td>
						<input id="processMaterialDetailList{{idx}}_qualityProblem" name="processMaterialDetailList[{{idx}}].qualityProblem" type="text" value="{{row.qualityProblem}}"  readonly="true"  class="form-control"/>
					</td>--%>
					...
					<td>
						<input id="processMaterialDetailList{{idx}}_changeBtn" type="button" onclick="changeDeal(this)"  class="form-control btn btn-primary" value="换件" />
					</td>
