	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/initbill/initBill/auditList"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		<div class="panel-body">
		<form:form  id="inputForm" modelAttribute="billMain" action="${ctx}/initbill/initBill/formAuditdo" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="tabs-container">
			...
			<script type="text/template" id="billDetailTpl">//<!--
				<tr id="billDetailList{{idx}}" onmousemove="getSum(this.id)">
					...
					<td>
						<input readOnly="readOnly onkeyup="calSum(this.value,this.id)"  id="billDetailList{{idx}}_itemPrice" name="billDetailList[{{idx}}].itemPrice" type="text" value="{{row.itemPrice}}"    class="form-control required"/>
					</td>
		...
		<c:if test="${fns:hasPermission('billmain:billMain:edit') || isAdd}">
		        ...
		        <div class="col-lg-6">
		             <div class="form-group text-center">
		                 <div>
		                     <button id="submitBtn" class="btn btn-primary btn-block btn-lg btn-parsley" data-loading-text="正在提交...">过 账</button>
		                 </div>
		             </div>
		        </div>
		</c:if>
