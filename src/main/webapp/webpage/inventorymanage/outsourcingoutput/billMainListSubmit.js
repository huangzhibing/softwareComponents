...
$(document).ready(function() {
	$('#billMainTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/outsourcingoutput/outsourcingOutput/data",
               ...
               queryParams : function(params) {
               	...
               	searchParam.searchWay="";//1代表需要根据登陆的人显示list 
               	searchParam.showWay="A";//N代表只显示带过账,A新增
                   return searchParam;
               },
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/outsourcingoutput/outsourcingOutput/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该单据记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/outsourcingoutput/outsourcingOutput/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/outsourcingoutput/outsourcingOutput/formSubmit?id="+row.id+"'>"+value+"</a>";
		         }
...
//		    }
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/outsourcingoutput/outsourcingOutput/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该单据记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/outsourcingoutput/outsourcingOutput/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#billMainTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/outsourcingoutput/outsourcingOutput/form?id=" + getIdSelections();
  }
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/outsourcingoutput/outsourcingOutput/detail?id="+row.id, function(billMain){
