...
<head>
	...
	function move(){
		...
		function isAllow(value,id){
			...
			var qty2=document.getElementById(id+"2").value;
			qty=parseFloat(qty);
			qty2=parseFloat(qty2);
			value=parseFloat(value);
			console.log(qty+" "+value);
			console.log(qty2+" "+id);
			var workFlag = '${billMain.workshopFlag}';
			if(subFlag==0){
				if('${isAdd}'=='true'||workFlag=='1'){
					if(value>qty){
						console.log("s+s");
		...
		function isAllowed(id){
			var value=document.getElementById(id+"_itemQty").value;
			var subFlag=document.getElementById("subFlag").value;
			var qty=document.getElementById(id+"_itemQty1").value;
			var qty2=document.getElementById(id+"_itemQty2").value;
			qty=parseFloat(qty);
			qty2=parseFloat(qty2);
			value=parseFloat(value);
			console.log(qty+" "+value);
		...
		function assign(value,id){
			var index=id.indexOf('_');
			var pre=id.substring(0,index);
			var ff=document.getElementById(pre+"_ff");
			if(ff.value==0){
				
				var resid=pre+"_itemQty2";
				document.getElementById(resid).value=document.getElementById(id).value;
				ff.value=1;
			}
		...
		function judgePrice() {
		    ...
		    for(var i in a){
		        if(i == 'length')
		            break;
                var pid = "billDetailList"+i+"_itemPrice";
                console.log(pid);
        ...
        function assignQty(){
            ...
            for(var i in a){
                if(i == 'length')
                    break;
                var pid = "billDetailList"+i+"_itemQty2";
                var p = document.getElementById(pid).value;
                if(p == '' || p == null || workFlag == 1){
                    document.getElementById(pid).value=0;
                }
            }
		}
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/outsourcingoutput/outsourcingOutput"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		<div class="panel-body">
		<form:form id="inputForm" modelAttribute="billMain" action="${ctx}/outsourcingoutput/outsourcingOutput/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
...
<%-- 						<form:select path="billFlag" class="form-control required" disabled="true">
 --%>							<input value="新增" readOnly="true"     class="form-control required"/><!-- 保存，新增，未提交 -->
								<form:input  style="display:none;" value="A" path="billFlag" htmlEscape="false"    class="form-control" />
						<%-- </form:select> --%>
					</div>
				</div>
				<div class="form-group">
					...
					<c:if test = "${billMain.billSource eq 'L'  }">
						<input  class="form-control" name="" readOnly="true" value="自动生成"/>

					</c:if>
					<c:if test = "${billMain.billSource eq 'M' || billMain.billSource eq null }">
						<input  class="form-control" name="" readOnly="true" value="手动输入"/>
						<input style="display: none;" value="M" name="billSource"/>
					</c:if>
					</div>
				</div>
				...
				<input style="display:none;" id="wareIdd" name="ware.wareID" value="${billMain.ware.wareID }" />
				<div class="form-group">
				...
				<form:input path="billPerson.id" style="display:none;"  id="billPerson" name="billPerson.no" value="${billMain.billPerson.no}" labelName="billPerson.no" labelValue="${billMain.billPerson.no}"
							    cssClass="form-control required"/>
				 <div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<sys:gridselect-modify extraField="wareEmpdd:user.no" disabled="disabled" labelField="user.no" targetId="wareEmpname" targetField="empName" url="${ctx}/employee/employee/data" id="wareEmp" name="wareEmp.id" value="${billMain.wareEmp.id}" labelName="wareEmp.user.no" labelValue="${billMain.wareEmp.user.no}"
				...
				<input style="display:none;" id="wareEmpdd" name="wareEmp.user.no" value="${billMain.wareEmp.user.no }"/>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<sys:gridselect-modify targetId="used" targetField="useDesc"
						extraField="useIdd:useId" url="${ctx}/use/use/data" id="invuse" name="invuse.id" value="${billMain.invuse.id}" labelName="invuse.useId" labelValue="${billMain.invuse.useId}"
				...
				<input style="display:none;" id="useIdd" name="invuse.useId" value="${billMain.invuse.useId }"/>
				<div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <form:select path="isGood" class="form-control">
		...
		<div class="tabs-container">
            ...
            <div class="tab-content">
				...
				<c:if test="${billMain.workshopFlag ne '1' }">
			...
			<script type="text/template" id="billDetailTpl">//<!--
				<tr id="billDetailList{{idx}}" onmousemove="getSum(this.id);isAllowed(this.id)">
					<td class="hide">
					...
					<td>
						<sys:gridselect-again
							...
							url="${ctx}/invaccount/invAccount/data" id="billDetailList{{idx}}_item" name="billDetailList{{idx}}.item.id" value="{{row.item.id}}" labelName="billDetailList{{idx}}.item.code" labelValue="{{row.item.id}}"
							 title="选择物料代码" cssClass="form-control required " fieldLabels="物料代码|物料名称|物料规格|数量" fieldKeys="item.code|item.name|item.specModel|realQty" searchLabels="物料代码" searchKeys="item.code" ></sys:gridselect-again>
					</td>
					...
					<td style="display:none;">
						<input  readOnly="readOnly" id="billDetailList{{idx}}_bin" name="billDetailList[{{idx}}].bin.binId" class="form-control  " value="{{row.bin.id}}" />
					</td>
				
					<td style="display:none;">
						<input readOnly="readOnly" id="billDetailList{{idx}}_loc" name="billDetailList[{{idx}}].loc.locId" type="text" value="{{row.loc.id}}"    class="form-control "/>
					</td>
					
					
					
					<td>
						<sys:gridselect-modify url="${ctx}/bin/bin/data"  urlParamName="wareId" urlParamId="wareNames" id="billDetailList{{idx}}_bin" name="billDetailList[{{idx}}].bin.id" value="{{row.bin.id}}" labelName="billDetailList{{idx}}.bin.binId" labelValue="{{row.bin.id}}"
							 ...
							 targetId="billDetailList{{idx}}_binName" targetField="binDesc" extraField="billDetailList{{idx}}_bin:binId"></sys:gridselect-modify>
					</td>
					...
					<td>
						<sys:gridselect-modify url="${ctx}/location/location/data" urlParamName="binId" urlParamId="billDetailList{{idx}}_binNames" id="billDetailList{{idx}}_loc" name="billDetailList[{{idx}}].loc.id" value="{{row.loc.id}}" labelName="billDetailList{{idx}}.loc.locId" labelValue="{{row.loc.id}}"
							 ...
							 targetId="billDetailList{{idx}}_locName" targetField="locDesc" extraField="billDetailList{{idx}}_loc:locId"></sys:gridselect-modify>
					</td>
					...
					<td>
						<input onkeyup="calSum(this.value,this.id)"  id="billDetailList{{idx}}_itemPrice" name="billDetailList[{{idx}}].itemPrice" type="text" value="{{row.itemPrice}}"    class="form-control "/>
					</td>
					
					<td>
						<input onkeydown="assign(this.value,this.id)" onkeyup="isAllow(this.value,this.id)" onkeyup="getSum(this.value,this.id)" id="billDetailList{{idx}}_itemQty" name="billDetailList[{{idx}}].itemQty" type="text" value="{{row.itemQty}}"    class="form-control required"/>
					</td>
					<td style="display:none;">
						<input   id="billDetailList{{idx}}_itemQty2" name="billDetailList[{{idx}}].itemQty2" type="text" value="{{row.itemQty}}"    class="form-control required"/>
					</td>
					<td style="display:none;">
						<input   id="billDetailList{{idx}}_ff" value="0"/>
					</td>
					<td style="display:none">
						<input   id="billDetailList{{idx}}_itemQty1" name="billDetailList[{{idx}}].itemQty1" type="text" value="{{row.itemQtyTemp}}"    class="form-control "/>
					</td>
		...
		<c:if test="${fns:hasPermission('billmain:billMain:edit') || isAdd}">
		        ...
		        <div class="col-lg-6">
		             <div class="form-group text-center">
		                 <div>
		                     <button onmousemove="judgePrice();assignQty()" id="submitBtn" class="btn btn-primary btn-block btn-lg btn-parsley" data-loading-text="正在提交...">保 存</button>
		                 </div>
		             </div>
		        </div>
		</c:if>
