...
$(document).ready(function() {
	$('#binTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/bin/bin/form?id=" + row.id;
	  ...
	  $('#binTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#remove').prop('disabled', ! $('#binTable').bootstrapTable('getSelections').length);
            $('#edit').prop('disabled', $('#binTable').bootstrapTable('getSelections').length!=1);
        });
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该货区记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/bin/bin/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#binTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/bin/bin/form?id=" + getIdSelections();
  }
