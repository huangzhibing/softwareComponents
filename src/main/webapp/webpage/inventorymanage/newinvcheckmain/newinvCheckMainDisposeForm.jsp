	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/newinvcheckmain/newinvCheckMain/Dispose"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		<div class="panel-body">
		<form:form id="inputForm" modelAttribute="newinvCheckMain" action="${ctx}/newinvcheckmain/newinvCheckMain/disposeF" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>	
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<sys:gridselect-modify url="${ctx}/warehouse/wareHouse/data" id="ware" name="wareId.id" value="${newinvCheckMain.wareId.id}" labelName="wareId.wareID" labelValue="${newinvCheckMain.wareId.wareID}"
		...
		<div class="tabs-container">
			...
			<script type="text/template" id="newinvCheckDetailTpl">//<!--
				<tr id="newinvCheckDetailList{{idx}}">
					...
					<td>
						<input id="newinvCheckDetailList[{{idx}}]_itemCode" name="newinvCheckDetailList[{{idx}}].itemCode.code" value="{{row.itemCode.code}}" readonly="readonly" class="form-control" />
					</td>
					...
					<td>
						<input id="newinvCheckDetailList{{idx}}_hgQty" name="newinvCheckDetailList[{{idx}}].hgQty" type="text" value="{{row.hgQty}}"    class="form-control required"/>
					</td>
		...
		<c:if test="${fns:hasPermission('newinvcheckmain:newinvCheckMain:edit') || isAdd}">
		        ...
		        <div class="col-lg-6">
		             <div class="form-group text-center">
		                 <div>
		                     <button class="btn btn-primary btn-block btn-lg btn-parsley" data-loading-text="正在处理...">处 理</button>
		                 </div>
		             </div>
		        </div>
		</c:if>
