...
$(document).ready(function() {
	$('#locationTable').bootstrapTable({
	  ...
	  $('#locationTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#remove').prop('disabled', ! $('#locationTable').bootstrapTable('getSelections').length);
            $('#edit').prop('disabled', $('#locationTable').bootstrapTable('getSelections').length!=1);
        });
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该货位记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/location/location/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#locationTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/location/location/form?id=" + getIdSelections();
  }
