<%@ page contentType="text/html;charset=UTF-8" %>
    ...
    $(document).ready(function() {
        $('#billMainTable').bootstrapTable({
            ...
            url: "${ctx}/inboundinput/inboundInput/data?type=${type}",
            ...
            onContextMenuItem: function(row, $el){
                if($el.data("item") == "edit"){
                    window.location = "${ctx}/inboundinput/inboundInput/form?id=" + row.id;
                } else if($el.data("item") == "delete"){
                    jp.confirm('确认要删除该单据记录吗？', function(){
                        jp.loading();
                        jp.get("${ctx}/inboundinput/inboundInput/delete?id="+row.id, function(data){
                            if(data.success){
            ...
            columns: [
                ...
                ,{
                    ...
                    ,formatter:function(value, row , index){
                        if('${type}' == 'Input'){
                            return "<a href='${ctx}/inboundinput/inboundInput/form?type=Input&id="+row.id+"'>"+value+"</a>";
                        }
                        if('${type}' == 'Query'){
                            return "<a href='${ctx}/inboundinput/inboundInput/form?type=Query&id="+row.id+"'>"+value+"</a>";
                        }
                        if('${type}' == 'Post'){
                            return "<a href='${ctx}/inboundinput/inboundInput/form?type=Post&id="+row.id+"'>"+value+"</a>";
                        }
                        if('${type}' == 'Fan'){
                            return "<a href='${ctx}/inboundinput/inboundInput/form?type=Fan&id="+row.id+"'>"+value+"</a>";
                        }
        ...
        $("#btnImport").click(function(){
            jp.open({
                ...
                btn1: function(index, layero){
                    window.location='${ctx}/inboundinput/inboundInput/import/template';
                },
    ...
    function deleteAll(){
        ...
        jp.confirm('确认要删除该单据记录吗？', function(){
            jp.loading();
            jp.get("${ctx}/inboundinput/inboundInput/deleteAll?ids=" + getIdSelections(), function(data){
                if(data.success){
                    $('#billMainTable').bootstrapTable('refresh');
                    jp.success(data.msg);
                }else{
    ...
    function edit(){
        window.location = "${ctx}/inboundinput/inboundInput/form?id=" + getIdSelections();
    }
...
//     });
    ...
    function print(){
        ...
        $("#printIframe").attr("src","${ctx}/inboundinput/inboundInput/printForm?id=" + getIdSelections());
