 ...
 //  var url="${ctx}/purreport/purReportSubmit/data";
	...
	$('#purReportTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/purreport/sfcReportSubmit/data",
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该检验单记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/purreport/sfcReportSubmit/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#purReportTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/purreport/sfcReportSubmit/form?id=" + getIdSelections();
  }
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/purreport/sfcReportSubmit/detail?id="+row.id, function(purReport){
