...
<div class="row">
	...
	<div class="panel panel-primary">
		...
		<form:form id="inputForm" modelAttribute="qmReportQuery" action="${ctx}/qmreportquery/qmReportQuery/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="tabs-container">
			...
			<div class="table-responsive">
			  <table class="table table-striped table-bordered table-condensed" style="min-width:150%" >
				<thead>
					<tr>
						<th class="hide"></th>
						<c:if test="${qmReportQuery.qmType=='整机'}">
							<th>机器序列号</th>
						</c:if>
						<c:if test="${qmReportQuery.qmType!='整机'}">
							<th>检验单编号</th>
			...
			<script type="text/template" id="qmReportRSnQueryTpl">//<!--
				<tr id="qmReportRSnQueryList{{idx}}">
					...
					<c:if test="${qmReportQuery.qmType=='整机'}">
					<td>
						<input id="qmReportRSnQueryList{{idx}}_matterType" name="qmReportRSnQueryList[{{idx}}].matterType" type="text" value="{{row.matterType}}"  readonly='true'  class="form-control "/>
					</td>
					</c:if>
					...
					<td class="text-center" width="10">
						{{#delBtn}}<span class="close" onclick="delRow(this, '#qmReportRSnQueryList{{idx}}')" title="删除">&times;</span>{{/delBtn}}
					</td>
				</tr>//-->
			</script>
			<script type="text/javascript">
