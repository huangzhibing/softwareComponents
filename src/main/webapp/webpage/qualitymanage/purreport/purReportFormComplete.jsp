...
<head>
	...
	<script type="text/javascript">
		...
		function sub(){
			...
			var newUrl = "${ctx}/purreport/purReport/subsaveCom";    //设置新提交地址
...
<div class="row">
	...
	<div class="panel panel-primary">
		...
		<form:hidden path="userDeptCode" value="${fns:getUserOfficeCode()}"/>
				...
				<div class="form-group" style="display:none">
					<label class="col-sm-2 control-label"><font color="red">*</font>整机扫码录入：</label>
					<div class="col-sm-10">
						<form:input path="justifyRemark" htmlEscape="false"  onchange="autoIn(this)"  class="form-control "/>
					</div>
				</div>
				...
				<div class="form-group">
					...
					-->
					<input id="sfcInvCheckMainbillNo" name="sfcInvCheckMain.billNo" value="${purReport.sfcInvCheckMain.billNo}" htmlEscape="false"  readonly="true"   class="form-control required">
					
					<!--  
						<sys:gridselect url="${ctx}/sfcinvcheckmain/sfcInvCheckMain/data" id="sfcInvCheckMain" name="sfcInvCheckMain.id" value="${purReport.sfcInvCheckMain.id}" labelName="sfcInvCheckMain.billNo" labelValue="${purReport.sfcInvCheckMain.billNo}"
			...
			<script type="text/template" id="purReportRSnTpl">//<!--
				<tr id="purReportRSnList{{idx}}">
					...
					<td class="hide">
						...
						<input id="purReportRSnList[{{idx}}].checkResult" name="purReportRSnList[{{idx}}].checkResult" type="hidden" value="合格"/>
					</td>
					
					<td style="width: 120px;">
						<input id="purReportRSnList{{idx}}_objSn" name="purReportRSnList[{{idx}}].objSn" type="text" value="{{row.objSn}}"  onchange="changeNum(this)"  class="form-control "/>
					</td>

					<td style="width: 120px;">
						<input id="purReportRSnList{{idx}}_mserialno" name="purReportRSnList[{{idx}}].mserialno" type="text" value="{{row.mserialno}}"    class="form-control "/>

					</td>
					
					
					<td style="width: 160px;">
						<input id="purReportRSnList{{idx}}_objCode" name="purReportRSnList[{{idx}}].objCode" type="text" value="{{row.objCode}}"    class="form-control "/>
					</td>
					
					
					<td style="width: 100px;">
						<input id="purReportRSnList{{idx}}_objName" name="purReportRSnList[{{idx}}].objName"  type="text" value="{{row.objName}}"    class="form-control "/>
					</td>
		        ...
		        <div class="col-lg-2">
		             <div class="form-group text-center">
		                ...
		                --%>
							...
							<input type="submit" class="btn btn-primary btn-block btn-lg btn-parsley" value="提交检验单" onclick="changefg()"> &nbsp;
		                 </div>
		             </div>
		        </div>
		...
		function changeNum(obj){
			...
			$.ajax({
				type:'POST',
				datatype:'text',
				url:"${ctx}/purreport/purReport/ajaxForObjNameCom",
				data:{"objCode":objCode,"objSn":value},
				contentType:"application/x-www-form-urlencoded",
				success:function(data){
					// alert(data);
					if(data=='1'){
			...
			var mCode=obj.value.substring(0,15);
			$("#"+"sfcInvCheckMainbillNo").val(mCode);
			//截取字符串
			if(value.length>=40){
				...
				$.ajax({
					...
					success:function(data){
						...
						$("#purReportRSnList0_objName").val(data.split("|")[0]);
						$("#purReportRSnList0_mserialno").val(data.split("|")[1]);
						$("#purReportRSnList0_objCode").val(objCode);
		...
		function autoIn(obj){
			...
			//alert(sn);
			if(sn.length>=20){
				mCode = sn.substring(0,15);
				code = sn.substring(15,27);
				alert(id);
				console.log($("#"+id));
				$("#"+id).val(mCode);
				$("#purReportRSnList0_objCode").val(code);
				//purReportRSnList{{idx}}_objCode
				//$("#"+obj.id).val(code);
			}
			//var objCodeId = "purReportRSnList0_objCode";
			//var objNameId = "purReportRSnList0_objName";
			$.ajax({
				type:'POST',
				datatype:'text',
				url:"${ctx}/purreport/purReport/getComFormAjax",
				//data:$("form").serializeArray(),
				data:{"code":code},
				contentType:"application/x-www-form-urlencoded",
				success:function(data1){
					//alert("ajax1");
					alert(data1);
					var purReportRSnRowIdx = 0;
					//$("#purReportRSnList").html("");
					//var data = ${fns:toJson(data1.purReportRSnList)};
					var data = data1.purReportRSnList;
					console.log(data1);
					alert(data);
					console.log(data[0]);
					addRow('#purReportRSnList', purReportRSnRowIdx, purReportRSnTpl, data[0]);
