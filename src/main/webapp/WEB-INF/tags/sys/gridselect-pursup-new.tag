...
$(document).ready(function(){
	$("#${id}Button, #${id}Names").click(function(){
		...
		top.layer.open({
			...
			labelName.splice(0,1);
			// var labelN="${labelName}".substring(labelName[0]);
			 ...
			 if(labelName.length==3){
				 ...
				 //获取隐藏标签所对应对象的属性名的
				 var  haddenName="${name}".split(".");
				 haddenName.splice(0,1);
				 if(haddenName.length==3){
	...
	function addValue(items,targetField,targetId){				
			for(var i=0;i<targetField.length;i++){
				...
				var tId=targetId[i];
				var tagId="#${id}".split("_")[0];
				$(tagId+"_"+tId).val(items[0][ind]);
			}
