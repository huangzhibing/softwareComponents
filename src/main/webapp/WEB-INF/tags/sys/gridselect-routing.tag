...
<%@ attribute name="urlParamId" type="java.lang.String" required="false" description="参数Id" %>
...
$(document).ready(function(){
	$("#${id}Button, #${id}Names").click(function(){
		...
		top.layer.open({
		    ...
		    auto:true,
		    name:'friend',
		    content: "${ctx}/outboundinput/outboundInput/gridselect?url="+encodeURIComponent("${url}")+urlParams+"&fieldLabels="+encodeURIComponent("${fieldLabels}")+"&fieldKeys="+encodeURIComponent("${fieldKeys}")+"&searchLabels="+encodeURIComponent("${searchLabels}")+"&searchKeys="+encodeURIComponent("${searchKeys}")+"&isMultiSelected=${isMultiSelected? true:false}",
		    btn: ['确定', '关闭'],
