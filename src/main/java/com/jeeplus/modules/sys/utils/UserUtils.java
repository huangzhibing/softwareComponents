...
public class UserUtils {
	...
	public static User getUser(){
		...
		if (principal!=null){
			...
			if (user != null){
				Office office=officeMapper.get(user.getOffice());  //从数据库中取出当前登录用户完整的归属部门信息
	 ...
	 * 获取当前用户所归属的部门编号
	 */
	public static String getUserOfficeCode() {
		User user=getUser();
		if(user!=null) {
			Office office=user.getOffice();
			office=officeMapper.get(office);
			if(office!=null) {
				user.setOffice(office);
	 ...
	 * 获取当前用户所在部门的编号
	 */
	public static String getUserOfficeName(){
		User user = getUser();
