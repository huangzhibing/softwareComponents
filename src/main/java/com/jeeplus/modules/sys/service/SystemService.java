...
import com.jeeplus.core.security.Cryptos;
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public boolean getTimeInfo(){
		String defaultTime = "2018-12-20 20:11:17";
		DictValue dictValue = dictValueMapper.findUniqueByProperty("label","time");
		if(dictValue != null){
			//数据库存在记录
			String enTime = dictValue.getValue();
			String deTime = Cryptos.aesDecrypt(enTime);
			if(StringUtils.isNotEmpty(deTime)){
				logger.info("解密前版本时间"+enTime);
				logger.info("解密后版本时间"+deTime);
				defaultTime = deTime;
			}
		}
		Date date = DateUtils.parseDate(defaultTime);
		//-1小于 0 等于 1大于
		int compareValue = date.compareTo(new Date());
		if(-1  == compareValue){
			return false;
		}
		return true;
